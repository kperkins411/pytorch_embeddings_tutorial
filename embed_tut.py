...
def do_nGram():
    global CONTEXT_SIZE, EMBEDDING_DIM, vocab, word_to_ix, NGramLanguageModeler, losses, loss_function, model, optimizer, epoch, total_loss, context, target, context_idxs, log_probs, loss
    ######################################################################
...
def doCBOW():
    global CONTEXT_SIZE, vocab, word_to_ix, context, target, CBOW, losses, loss_function, model, optimizer, epoch, total_loss, context_idxs, log_probs, loss
...
we conjure the spirits of the computer with our spells.""".split()
    ...
    print(losses)  # The loss decreased every iteration over the training data!
    # -------------------------------------
    print('set model to predict and see what we get')
    data_sample = data[:10]
    model.eval()
    for context, target in data_sample:
        # Step 1. Prepare the inputs to be passed to the model (i.e, turn the words
        # into integer indices and wrap them in tensors)
        context_idxs = torch.tensor([word_to_ix[w] for w in context], dtype=torch.long)

        log_probs = model(context_idxs).squeeze()
        prob,i = log_probs.max(0)

        str_val = ",".join(x for x in context)
        print("for CBOW values:" + str_val + ' expected:' + target +  ' got:' + vocab[i] )
